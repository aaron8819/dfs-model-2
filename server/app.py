...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        with engine.connect() as db:
            check(db, production=not settings.local_test)
        yield
        completion.close()
        engine.dispose()
    ...
    async def headers(
        ...
        if response.status_code >= 400:
            logging.getLogger(__name__).warning(
                "http_rejected status=%s method=%s", response.status_code, request.method
            )
    ...
    def readiness() -> Response:
        try:
            with engine.connect() as db:
                state = check(db, production=not settings.local_test)
                clock = one(
                    db, "SELECT observed_at <= clock_timestamp() AS healthy FROM clock_observation"
                )
                ready = not state["recovery_required"] and clock["healthy"]
            return JSONResponse(
                {"status": "ready" if ready else "blocked"}, status_code=200 if ready else 503
            )
        except Exception:
    ...
    @app.get("/api/workspaces/{wid}/completions")
    def recent_completions(wid: UUID, session: dict = Depends(auth.require)) -> list[dict]:
        service.read(str(wid), str(session["owner_id"]))
        with engine.connect() as db:
            return many(
                db,
                "SELECT id,created_at FROM recommendation_request WHERE workspace_id=:wid "
